...
class MDIOSPI:
    ...
    def open(self, speed_hz: int=5000):
        ''' Open mdio bus. '''
        if len(re.findall(r'[0-9]+.[0-9]+', self.path)) != 2:
            raise Exception('Invalid resource path given. Expected format /dev/spidev##.##')
        bus_id, device_id = re.findall(r'[0-9]+.[0-9]+', self.path)[0].split('.')
        self._bus.open(int(bus_id), int(device_id))
