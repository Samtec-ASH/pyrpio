...
static PyObject *py_mdio_c22_read(PyObject *self, PyObject *args) {
  ...
  uint16_t val;
  if (!PyArg_ParseTuple(args, "BBBB", &clk_pin, &data_pin, &pad, &reg)) { return NULL; }
...
static PyObject *py_mdio_c22_write(PyObject *self, PyObject *args) {
  uint8_t clk_pin, data_pin, pad, reg;
  uint16_t val, rst;
  if (!PyArg_ParseTuple(args, "BBBBH", &clk_pin, &data_pin, &pad, &reg, &val)) { return NULL; }
...
static PyObject *py_mdio_c45_read(PyObject *self, PyObject *args) {
  ...
  uint16_t reg, val;
  if (!PyArg_ParseTuple(args, "BBBBH", &clk_pin, &data_pin, &pad, &dad, &reg)) { return NULL; }
...
static PyObject *py_mdio_c45_write(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "BBBBHH", &clk_pin, &data_pin, &pad, &dad, &reg, &val)) { return NULL; }
